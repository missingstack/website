...
export function StackContent({
	...
	const fetchTools = async ({ pageParam }: { pageParam?: string }) => {
		// Build query options for the stack endpoint
		// stackId is in the URL path, so we only pass query parameters
							...
							>
								...
								{!hasNextPage && allTools.length > 0 && !isFetchingNextPage && (
									<p className="text-muted-foreground text-xs sm:text-sm">
										...
										{allTools.length !== 1 ? "s" : ""} in this stack
									</p>
								)}
							</div>
						</>
